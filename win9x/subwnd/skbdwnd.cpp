...
void CSoftKeyboardWnd::Create()
{
	...
	if (!softkbd_getsize(&m_nWidth, &m_nHeight))
	{
		return;
	}

	if (!CSubWndBase::Create(IDS_CAPTION_SOFTKBD, WS_OVERLAPPED | WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX, s_skbdcfg.posx, s_skbdcfg.posy, m_nWidth, m_nHeight, NULL, NULL))
	{
		return;
	}
...
LRESULT CSoftKeyboardWnd::WindowProc(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		case WM_CREATE:
			SetClientSize(m_nWidth, m_nHeight);
			break;
