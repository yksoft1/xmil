...
static const TCHAR s_crlf[] = TEXT("\r\n");

static TRACECFG s_tracecfg;

static const TCHAR s_inifile[] = TEXT("xmiltrace.ini");
static const TCHAR s_inititle[] = TEXT("TRACE");
static const PFTBL s_initbl[4] =
{
	PFVAL("posx",	PFTYPE_SINT32,	&s_tracecfg.posx),
	PFVAL("posy",	PFTYPE_SINT32,	&s_tracecfg.posy),
	PFVAL("width",	PFTYPE_SINT32,	&s_tracecfg.width),
	PFVAL("height",	PFTYPE_SINT32,	&s_tracecfg.height)
};
...
#else
	m_nFlags = 1;
	m_tfh = textfile_create(TEXT("traceout.txt"), 0x800);
#endif

	s_tracecfg.posx = CW_USEDEFAULT;
	s_tracecfg.posy = CW_USEDEFAULT;
	s_tracecfg.width = CW_USEDEFAULT;
	s_tracecfg.height = CW_USEDEFAULT;
	ini_read(file_getcd(s_inifile), s_inititle, s_initbl, _countof(s_initbl));

	if (!CreateEx(WS_EX_CONTROLPARENT, s_szClassName, s_szTitle, WS_OVERLAPPEDWINDOW, s_tracecfg.posx, s_tracecfg.posy, s_tracecfg.width, s_tracecfg.height, NULL, NULL))
	{
		return;
	}
	ShowWindow(SW_SHOW);
...
void CTraceWnd::Deinitialize()
{
	...
	DestroyWindow();
	ini_write(file_getcd(s_inifile), s_inititle, s_initbl, _countof(s_initbl));
}
	...
	switch (nMsg)
	{
		...
		case WM_MOVE:
			if (!(GetStyle() & (WS_MAXIMIZE | WS_MINIMIZE)))
			{
				RECT rc;
				GetWindowRect(&rc);
				s_tracecfg.posx = rc.left;
				s_tracecfg.posy = rc.top;
			}
			break;
		...
		case WM_SIZE:							// window resize
			if (!(GetStyle() & (WS_MAXIMIZE | WS_MINIMIZE)))
			{
				RECT rc;
				GetWindowRect(&rc);
				s_tracecfg.width = rc.right - rc.left;
				s_tracecfg.height = rc.bottom - rc.top;
			}
